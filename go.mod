module github.com/gidoBOSSftw5731/rpi-thermostat

go 1.21

require github.com/gidoBOSSftw5731/log v0.0.0-20210527210830-1611311b4b64

require (
	github.com/d2r2/go-dht v0.0.0-20200119175940-4ba96621a218 // indirect
	github.com/d2r2/go-logger v0.0.0-20210606094344-60e9d1233e22 // indirect
	github.com/d2r2/go-shell v0.0.0-20211022052110-f591c27e3e2e // indirect
	github.com/davecgh/go-spew v1.1.1 // indirect
	github.com/gidoBOSSftw5731/goweather v0.0.0-20240305235943-a0b1eeae0b8b // indirect
	github.com/warthog618/gpiod v0.8.2 // indirect
	golang.org/x/sys v0.10.0 // indirect
)
